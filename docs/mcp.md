...
Ootrix CRM ships a [Model Context Protocol](https://modelcontextprotocol.io)
...
[`ootrix-crm-mcp`](https://www.npmjs.com/package/ootrix-crm-mcp). Under the hood
...
data or send messages, add `"OOTRIX_CRM_ENABLE_WRITES": "true"` (and
`"OOTRIX_CRM_ENABLE_BROADCASTS": "true"` for mass sends) to `env`.
