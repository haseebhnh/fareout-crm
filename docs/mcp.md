...
Fareout CRM ships a [Model Context Protocol](https://modelcontextprotocol.io)
...
[`fareout-crm-mcp`](https://www.npmjs.com/package/fareout-crm-mcp). Under the hood
...
data or send messages, add `"FAREOUT_CRM_ENABLE_WRITES": "true"` (and
`"FAREOUT_CRM_ENABLE_BROADCASTS": "true"` for mass sends) to `env`.
