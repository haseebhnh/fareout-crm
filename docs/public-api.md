...
The public API lets you drive your Ootrix CRM instance from your own
...
```
Authorization: Bearer ootrix_crm_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** Ootrix CRM
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer ootrix_crm_live_xxx"
```
...
Rather than polling, register an endpoint and Ootrix CRM will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; Ootrix CRM keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer ootrix_crm_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/ootrix-crm", "events": ["message.received"] }'
...
Headers: `X-Ootrix-Crm-Event`, `X-Ootrix-Crm-Webhook-Id`, and `X-Ootrix-Crm-Signature`.

### Verifying the signature

`X-Ootrix-Crm-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages Ootrix CRM stores (inbox + API sends), not broadcast-only
...
[#245](https://github.com/ArnasDon/ootrix-crm/issues/245). Future ideas
