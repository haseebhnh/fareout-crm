...
The public API lets you drive your Fareout CRM instance from your own
...
```
Authorization: Bearer fareout_crm_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** Fareout CRM
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer fareout_crm_live_xxx"
```
...
Rather than polling, register an endpoint and Fareout CRM will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; Fareout CRM keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer fareout_crm_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/fareout-crm", "events": ["message.received"] }'
...
Headers: `X-Fareout-Crm-Event`, `X-Fareout-Crm-Webhook-Id`, and `X-Fareout-Crm-Signature`.

### Verifying the signature

`X-Fareout-Crm-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages Fareout CRM stores (inbox + API sends), not broadcast-only
