...
describe("verifyMetaWebhookSignature", () => {
  ...
  // The verifier no longer reads process.env — the caller resolves which
  // secret applies (account-level, else global) and passes it in. So
  // "missing secret" is now expressed as a null/undefined argument, and
  // fail-closed means rejecting even a validly-signed body.
  describe("fail-closed when no secret is available", () => {
    it.each([
      ["null", null],
      ["undefined", undefined],
      ["empty string", ""],
    ])("rejects a correctly-formed signature when secret is %s", (_, secret) => {
      const body = "{}";
